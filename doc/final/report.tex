...
% The paper headers
\markboth{Journal of \LaTeX\ Class Files,~Vol.~12, No.~4, December~2017}%
...
\begin{abstract}
Image classification is one of the most interesting and challenging tasks in computer vision. While most of the work in this area has focused on single-label image classification where each image has only one class label, it is important to remember that real world images typically have multiple labels. The standard method for solving multi-label image classification is to train multiple independent classifiers, one for each category, and apply thresholding or ranking across the classification results. This method works well but fails to capture the strong co-occurrence dependencies of labels on an image. Deep Neural Networks have great potential to capture these types of label relationships and ultimately boost the overall performance of a classifier. In this work we will attempt to build such a classifier and compare our results to the latest models. The purpose of this project is not to present original research but rather to demonstrate a working knowledge of Machine Learning algorithms and techniques.    
\end{abstract}

% Note that keywords are not normally used for peerreview papers.
\begin{IEEEkeywords}
Machine Learning, Deep Learning, Transfer Learning, Neural Networks, Keras, Tensorflow, VGG16, MS-COCO.
\end{IEEEkeywords}}
